...
export default function PresupuestoVenta({ proyectoId }) {
  ...
  const calcularMateriales = () => {
    ...
    const precios = JSON.parse(localStorage.getItem(preciosKey) || '{}');
  ...
  const calcularMO = () => {
    ...
    const total = moData.reduce((sum, item) => sum + (item.subtotal || 0), 0);
    setTotalMO(total);
  };

  const costoNeto = totalMateriales + totalMO;
  const impuestosCalc = costoNeto > 0 ? (costoNeto * impuestos) / 100 : 0;
  const totalConGastos = costoNeto + impuestosCalc + gastosEstructura + otrosGastos;
  const ganancia = totalConGastos > 0 ? (totalConGastos * margenGanancia) / 100 : 0;
  const totalSinIVA = totalConGastos + ganancia;
  const ivaCalc = totalSinIVA > 0 ? totalSinIVA * 0.21 : 0;
  const totalConIVA = totalSinIVA + ivaCalc;
  ...
  return (
    <div style={{ background: 'white', padding: '32px', borderRadius: '12px', boxShadow: '0 4px 12px rgba(0,0,0,0.08)' }}>
      ...
      <p style={{ color: '#666', marginBottom: '24px' }}>Resumen ejecutivo con gastos e impuestos</p>
      ...
      <div style={{ background: '#f9fafb', padding: '20px', borderRadius: '10px', marginBottom: '24px', border: '1px solid #e5e7eb' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '4px', color: '#666', fontSize: '12px', fontWeight: '600' }}>Impuestos (%)</label>
            <input type="number" value={impuestos} onChange={(e) => setImpuestos(parseFloat(e.target.value) || 0)} style={{ width: '100%', padding: '10px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '13px', boxSizing: 'border-box' }} />
          </div>

          <div>
            <label style={{ display: 'block', marginBottom: '4px', color: '#666', fontSize: '12px', fontWeight: '600' }}>Gastos de Estructura ($)</label>
            <input type="number" step="0.01" value={gastosEstructura} onChange={(e) => setGastosEstructura(parseFloat(e.target.value) || 0)} style={{ width: '100%', padding: '10px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '13px', boxSizing: 'border-box' }} />
          </div>

          <div>
            <label style={{ display: 'block', marginBottom: '4px', color: '#666', fontSize: '12px', fontWeight: '600' }}>Otros Gastos ($)</label>
            <input type="number" step="0.01" value={otrosGastos} onChange={(e) => setOtrosGastos(parseFloat(e.target.value) || 0)} style={{ width: '100%', padding: '10px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '13px', boxSizing: 'border-box' }} />
          </div>

          <div>
            <label style={{ display: 'block', marginBottom: '4px', color: '#666', fontSize: '12px', fontWeight: '600' }}>Margen de Ganancia (%)</label>
            <input type="number" value={margenGanancia} onChange={(e) => setMargenGanancia(parseFloat(e.target.value) || 0)} style={{ width: '100%', padding: '10px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '13px', boxSizing: 'border-box' }} />
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
        ...
        <div style={{ background: '#f9fafb', padding: '16px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
          ...
          <h3 style={{ margin: 0, color: '#1c2d4f', fontSize: '20px' }}>${ivaCalc.toFixed(2)}</h3>
        </div>
